...
def bert_span_mask(arr, mask_percentage=0.15, max_span_length=600, p=0.2):
    """
    ...
    # Step 1: Sample spans to cover approximately 15% of the array
    spans = []
    covered = 0
    while covered < num_mask:
        word_length = np.random.geometric(p)
        word_span_length = word_length * 60
        span_length = np.random.randint(1, word_span_length)
        span_length = min(span_length, max_span_length, num_mask - covered)

        start = np.random.randint(0, total_elements - span_length + 1)

        # Check if span overlaps with already selected spans
        if any(s_start <= start < s_end or s_start < start + span_length <= s_end for s_start, s_end in spans):
            continue

        spans.append((start, start + span_length))
        covered += span_length

    # Flatten the spans to get a list of all the indices directly into mask_indices
    mask_indices = np.array([idx for start, end in spans for idx in range(start, end)])

    # Shuffle and apply 80-10-10 on the total span indices
    #print(mask_indices)
...
def get_input_sample(sent_obj, tokenizer, max_len=56, add_CLS_token=False, max_spectro_datapoint=24000):
    ...
    input_sample['masked_EEG'], input_sample['mask_indices'] = bert_span_mask(sent_eeg_embedding, mask_percentage=0.15, max_span_length=600, p=0.2)
