...
def get_input_sample(sent_obj, tokenizer, eeg_type='GD', bands=['_t1', '_t2', '_a1', '_a2', '_b1', '_b2', '_g1', '_g2'],
    ...
    input_sample['normalized_input_embeddings'] = torch.stack(normalized_word_sentence_embeddings)  # max_len * (105*num_bands)
...
if __name__ == "__main__":
    ...
    # tokenizer = T5Tokenizer.from_pretrained('./models/huggingface/flan-t5-large')
    tokenizer = PegasusTokenizer.from_pretrained('./models/huggingface/pegasus-large')
        ...
        if subject != 'YMH':
            ...
            for i in range(train_divider):
                ...
                if input_sample is not None:
                    output_name = f"./datasets/data_word_sentence_3_pegasus/train/{version}-{task_name}-{subject}-{i}.pickle"
            ...
            for i in range(train_divider, dev_divider):
                input_sample = get_input_sample(dataset_dict[subject][i], tokenizer, eeg_type,
                                                bands=bands, add_CLS_token=is_add_CLS_token, max_len=max_len, dim=dim)
                if input_sample is not None:
                    output_name = f"./datasets/data_word_sentence_3_pegasus/valid/{version}-{task_name}-{subject}-{i}.pickle"
                    with codecs.open(output_name, 'wb') as handle:
                        pickle.dump(input_sample, handle, protocol=pickle.HIGHEST_PROTOCOL)
            ...
            for i in range(dev_divider, total_num_sentence):
                input_sample = get_input_sample(dataset_dict[subject][i], tokenizer, eeg_type,
                                                bands=bands, add_CLS_token=is_add_CLS_token, max_len=max_len, dim=dim)
                if input_sample is not None:
                    output_name = f"./datasets/data_word_sentence_3_pegasus/test/{version}-{task_name}-{subject}-{i}.pickle"
                    with codecs.open(output_name, 'wb') as handle:
                        pickle.dump(input_sample, handle, protocol=pickle.HIGHEST_PROTOCOL)
