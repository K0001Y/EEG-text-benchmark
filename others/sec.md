...
你的核心职责：在不改动章节框架的前提下，根据我后续的提问，将罗列式初稿改写为具批判性、连贯性与学术严谨性的综述文本。未经我明确同意，**不得新增、删除、合并或重命名任何章节标题**；不得调整章节顺序。
...
\section{Scope and Task Definitions} % 第2章：研究范畴与任务定义
    \subsection{EEG and open-vocabulary EEG-to-text decoding} % 2.1 非侵入式脑信号模态概览
    <!--
    段内思路，不再显式地划分paragraph：
    ...
    -->
    \subsection{Levels of open-vocabulary EEG-to-text decoding} % 2.2 解码任务粒度与多维视角
...
% 第3章：开放词表脑电转文本的技术演进
    \subsection{Sequence-to-Sequence Direct Mapping Paradigm} %3.1 早期序列到序列直接映射路径：
    ...
    <!--这一阶段的定义：将脑电特征直接映射到预训练的语言模型中-->
    <!--
        段落思路大致如下，不再显式分为小标题：
        第1、2段：总起介绍直接映射的范式，介绍这一阶段中提出 开放词汇脑电文本解码任务 的模型
        第3、4段：介绍编码器（CNN、RNN）
        第5、6段：介绍直接映射范式与语言模型解码
        第7段： 分析方法问题
    -->    
    ...
    <!--后期：引入大语言模型的相关方法进行性能提升-->
        \subsubsection{Representation Learning Integrated with Large Language Models} % 3.3.1结合大语言模型的特征学习    
        <!--小节内部不再分paragraph，但按照这个结构写：
        \paragraph{Parameter-Efficient Fine-tuning and Soft Prompting} % 参数高效微调(LoRA)与连续软提示注入
        -->
        \subsubsection{Generative Architectures and Decoding Paradigms} % 3.3.2 基于大语言模型的高级解码范式
        <!--小节内部不再分paragraph，但按照这个结构写：
...
\section{Evaluation Pitfalls in EEG-to-Text Generation}% 第4章：脑电转文本生成中的评估陷阱
    \subsection{Noise Baselines and Shuffled-Control Experiments} % 4.4 噪声基线与打乱对照实验 （参考实验报告中的B线实验）
    ...
    <!--指出 BLEU 和 ROUGE 来自机器翻译和文本摘要，不一定适合 EEG-to-text。EEG-to-text 不能只依赖 BLEU/ROUGE，需要结合语义检索、脑信号消融和人类评价。问题有：高频词和模板化句子会提高分数；BLEU 偏向表面 n-gram 匹配；低 BLEU 不一定表示语义失败；高 BLEU 也不一定说明 EEG 信息被使用；对短句和小数据集特别敏感。-->
    \subsection{Teacher Forcing and Exposure Bias} % 4.2 教师强制与暴露偏差引起的性能假象 （模型解码器是否真正依赖脑电模态）
    <!--重点讨论-->
    \subsection{Memorization Artifacts and the Failure of Representation Learning} %4.3 记忆伪影与表征学习的失效（模型是否学到了真实的神经特征？是否建立了真实的跨模态对齐）
